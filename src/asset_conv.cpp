  ...
  Processor(int n_threads = NUM_THREADS) : should_run_(true) {
    if (n_threads <= 0) {
      std::cerr << "Warning, incorrect number of threads (" << n_threads
                << "), setting to " << NUM_THREADS << std::endl;
...
int parseNumThreads(char *num_threads_str) {
  int num_threads = std::atoi(num_threads_str);
  if (num_threads == 0) {
    std::cerr << "num_threads \"" << num_threads_str
              << "\" couldn't be parsed, defaulting to 1." << std::endl;
    return 1;
  } else if (num_threads > 1000) {
    std::cerr << "num_threads higher than max 1000, defaulting to 1."
              << std::endl;
    return 1;
  }
  return num_threads;
